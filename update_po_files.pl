...
# 1.0.0    2024-02-03  sed, EdenWorX   Do not change systemd->elogind in systemd project or team URLs
#
# ========================
# === Little TODO list ===
# ========================
...
for my $pofile ( @source_files ) {
	...
	for my $line ( @lPo ) {
		...
		elsif ( ( $line =~ m/systemd/ ) &&
		        !( $line =~ m,fedoraproject\.org/projects, ) &&
		        !( $line =~ m,freedesktop\.org/wiki, ) &&
		        !( $line =~ m,freedesktop\.org/software, ) ) {
			$line =~ s/systemd(?:[_-]login)?(?:d)?/elogind/g;
		}
